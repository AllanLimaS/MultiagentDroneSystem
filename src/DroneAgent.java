...
class MoveToTarget extends Behaviour {
    ...
    @Override
    public int onEnd() {
        var product = Agent.Context.DronesTranform.stream().filter(x -> x.Id == Agent.uuid).findFirst().get().Product;
        if (product == 0){ // caso ele nao estiver carregando produto, ele pega o produto da fabrica.
            Agent.Context.DronesTranform.stream().filter(x -> x.Id == Agent.uuid).findFirst().get().Product =  Agent.Context.IndustryTranform.stream().filter(x -> x.Transform.Position == Agent.target).findFirst().get().Product;
        } else {//caso ele estiver produto, ele simplesmente solta o produto
            Agent.Context.DronesTranform.stream().filter(x -> x.Id == Agent.uuid).findFirst().get().Product = 0;
        }
        Agent.target = null;
        return 1;
    }
...
class GetTarget extends OneShotBehaviour{
    ...
    public void action() {
        ...
        if (product==0){ // não ta carregando produto, logo tem que ir para uma industria
            var maxIndex = Agent.Context.IndustryTranform.size() - 1;
            var index = generator.nextInt(0, maxIndex);
            Transform randomValue = Agent.Context.IndustryTranform.get(index).Transform;
            Agent.target = randomValue.Position;

        }else{ //caso ele estiver carregando um produto, ele tem que procurar um consumidor que queira o produto
            var maxIndex = Agent.Context.ConsumersTransform.size() - 1;
            var index = generator.nextInt(0, maxIndex);

            boolean ok = false;
            do{
                index = generator.nextInt(0, maxIndex);
                if(Agent.Context.ConsumersTransform.get(index).Product == product){
                    ok = true;
                }
